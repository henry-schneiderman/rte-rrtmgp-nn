# Based on RT_net_hws_9.py as of Aug. 14, 2023
# Does more general nn to estimate direct transfer coefficient
...
class LayerPropertiesDirect(Layer):
    ...
    def __init__(self, n_channels, **kargs):
        ...
        self.n_channels = n_channels
        self.net = DenseFFN([6,8,10],self.n_channels, 0.2, 0.8)

    def call(self, input): #, training=False):

        lw, h2o, o3, co2, u, n2o, ch4, t_p, mu = input
        concat_input = tf.concat([lw, h2o, o3, co2, u, n2o, ch4, t_p, mu], axis=1)
        #concat_input = tf.expand_dims(concat_input, axis=1)

        t_direct = self.net(concat_input)   #, training)
...
class DownwardPropagationCellDirect(Layer):
    def __init__(self,n_channels,**kwargs):
        super().__init__(**kwargs)
        self.n_channels = n_channels
        self.state_size = [tf.TensorShape([self.n_channels])]
        self.output_size = tf.TensorShape([self.n_channels])
...
def train():
    ...
    t_direct = TimeDistributed(LayerPropertiesDirect(n_channels), name="layer_properties")([lw, h2o, o3, co2, u, n2o, ch4, t_p, mu])
    ...
    #t_direct = tf.expand_dims(t_direct, axis=3)
