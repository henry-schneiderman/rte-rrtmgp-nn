...
if True:   #not final_evaluation:
    ...
    fpath = 'saved_model/bigru_gru_16_nocoldry_levoutput.HWS5.TEMP.5600'
    fpath = 'saved_model/bigru_gru_16_nocoldry_levoutput.HWS6.TEMP.6806'

    model = tf.keras.models.load_model(fpath) # + 'FINAL')
...
if evaluate_onnx:
    ...
    if True:
        #y_pred = newmodel({"inputs_aux_albedo": xaux, "inputs_main": x_main})

        y_pred = model({ "inputs_main": x_main,
                        "inputs_aux_albedo": xaux,
                        "target" : y_true,
                        "dpres" : dp,
                        "incflux": rsd0_big})

        """         y_pred = model.predict({ "inputs_main": x_main,
                        "inputs_aux_albedo": xaux,
                        "input_1" : y_true,
                        "input_2" : dp,
                        "input_3": rsd0_big}) """

        rmse_final = rmse_hr(y_true, y_pred, dp, rsd0_big)

        print("rmse error = " + str(rmse_final))
